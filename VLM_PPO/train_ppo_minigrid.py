"""
PPO Reinforcement Learning Training — Qwen2.5-VL on MiniGrid

Entry point for training Qwen2.5-VL models with Proximal Policy
Optimization (PPO) on MiniGrid environments (DoorKey-6x6, Empty-Random, etc.).

Supports:
  - LoRA fine-tuning of Qwen2.5-VL-3B-Instruct
  - Temperature predictor auxiliary head
  - Action-frequency reward shaping
  - RLEF (RL from Expert Feedback) mode
  - Checkpoint saving / resuming
  - DeepSpeed ZeRO-2 via Accelerate
  - Wandb logging with group/run tracking

Usage:
  See scripts/run_minigrid_qwen.sh for an example launch command.
"""
