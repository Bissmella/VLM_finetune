...
class LazyTrajSupervisedDataset(Dataset):
    """
    Trajectories dataset for finetuning.
    Dataset for supervised fine-tuning.
    """
    ...
    def __init__(self, data_path: str,
                 ...
                 data_args: DataArguments,
                 train=True):
        super(LazyTrajSupervisedDataset, self).__init__()
        list_data_dict = json.load(open(data_path, "r"))
        ...
        self.train = train
        self.data_args = data_args
        self.query = "You are an expert 2D game player in a grid-based environment. "
        self.query += "You are observing the image of the current state, and your goal is to get the player to the pink goal square/tile. "
        self.query += "The player is shown by cyan triangle.The tip (pointy end) of the triangle is the direction the player is facing, the flat side is the back. In the game the player must pick up a key to unlock the door. The square with a minus (-) and blue or yellow color is the closed door. The player should reach the pink goal tile to win. "
        self.query += "At each step the possible actions are: ['Turn left': turns direction to left, 'Turn right': turns direction to right, 'Move forward': take one step to front, 'Pick up': picks key only if key is in front of it, 'Toggle': toggle door to open it only if door is in front of it]. "
        self.query +=  "Your response should be a valid json object in the following format. Include 'thoughts' only if it is needed to reason: \{\n"
        self.query += "\"thoughts\": \"(Optional) describe the current scene and state of the agent as seen in the image.\", \n"  ##Describe the current scene step-by-step and explain the visible area, position and facing direction of the player, nearby objects, or interactive elements with their relative locations.
        self.query += "\"action\": \"your chosen action\" \n"
        self.query += "}"
            #) #TODO the main query where the {action} will be put in
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        img1_file = sources['image']
        #img2_file = sources['after_img']
        img1 = Image.open(img1_file) #.convert('RGB')
        #img2 = Image.open(os.path.join(image_folder, img2_file))
        img1 = self.transform(img1).permute(1,2, 0).unsqueeze(0)
        #img2 = self.transform(img2).permute(1,2, 0).unsqueeze(0)
        response = sources['action']
        action = sources['action']
        #util_score = sources['utility_score']
        # query = (
        #         "You're an expert in 2D grid-based games guiding a player by scoring his each step based. "
...
class Collate_fn_qwen(object):
    ...
    def __call__(self, examples):
        ...
        if isinstance(self.processor, Qwen2VLProcessor):  # Check if the processor is Qwen2VLProcessor
            ...
            image_tokens = [self.processor.tokenizer.convert_tokens_to_ids(self.processor.image_token)]  # Convert image token to ID
        # labels, batch_slices, random_coeffs = self.assign_random_mask_weights(labels, random_masks, bad_masks)
        pattern = torch.tensor([77091], device=labels.device)   #that is fixed token id of "assistant"
        match = (labels.unfold(1, len(pattern), 1) == pattern).all(-1)
...
def make_supervised_data_module(processor,
    ...
    full_dataset = LazyTrajSupervisedDataset(processor=processor,
...
def evaluate(base, evalset, processor):
    ...
    lora_weights = torch.load("/home/bahaduri/RL4VLM/outputs/sft_trajs/model_6.0.checkpoint", map_location='cpu')
    ...
    value_model.save_adapter("/home/bahaduri/RL4VLM/outputs/sft_trajs/lora_weights", "policy")
    breakpoint()
...
def main():
    ...
    if training_args.lora_enable:
        ...
        """
        # for saving the lora weights
        lora_weights = torch.load("/home/bahaduri/RL4VLM/outputs/sft_trajs/model_6.0.checkpoint", map_location='cpu')
        missing_keys, unexpected_keys = base.load_state_dict(lora_weights, strict=False)
        print("**********", len(unexpected_keys))
        base.save_pretrained("/home/bahaduri/RL4VLM/outputs/sft_trajs/lora_weights")
        """
