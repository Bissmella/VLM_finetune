...
def main():
    ...
    if "Qwen2.5" in model_path:
        ...
        hidden_dim = 1536
    value_model = QwenVLMValue(base, processor, hidden_dim, grpo = False) #args.grpo)  using value model even if grpo
    ###
    #for loading lora weights for testing purposes
    
    lora_weights = torch.load("/home/bahaduri/RL4VLM/outputs/dk_VLM_eps_1_grpo_q25_4/model_28.checkpoint", map_location='cpu')
    ...
    rollouts = RolloutStorage(500, args.num_processes,
                              envs.observation_space.shape, envs.action_space, args.max_new_tokens, temporal_predictor, args.act_freq_reward, scale=0.00025, grpo=False)#args.grpo)
